...
class Sprite:
    ...
    def collide(self, candidate):
        return (self.radius + candidate.radius) ** 2 > (self.pos[0] - candidate.pos[0]) ** 2 + \
                                                       (self.pos[1] - candidate.pos[1]) ** 2
...
def group_collide(group, candidate):
    to_remove = set()

    for item in group:
        if item.collide(candidate):
            to_remove.add(item)
...
def group_to_group_collide(group, colliding_group):
    to_remove = set()

    for item in colliding_group:
        if group_collide(group, item) > 0:
            to_remove.add(item)

    colliding_group.difference_update(to_remove)
    return len(to_remove)
...
def draw(canvas):
    ...
    # draw ship and sprites
        ...
        fired = group_to_group_collide(rock_group, missile_group)
...
def reset():
    global my_ship, rock_group, missile_group, lives, score, time
